...
public class CompressUtil {
     ...
     * @param baseDir
     */
    public static void compressToZip(File sourceFile, ZipOutputStream zipOut, String baseDir) throws IOException {
        // 文件夹的压缩
    ...
    public static void compressToTarGz(String sourcePath, String targetPath) {
        ...
        //3.删除tar
        if (!FileUtils.deleteFile(rarSourcePath)) {
    ...
    public static File validateSourcePath(String sourcePath) {
        File sourceFile = new File(sourcePath);
        if (!sourceFile.exists()) {
            System.out.println("the source file is not exist, source path: " + sourceFile.getAbsolutePath());
    ...
    public static File validateTargetPath(String targetPath) {
        ...
        if (!targetFile.exists()) {
            System.out.println("the target file is not exist, target path: " + targetFile.getAbsolutePath() + ". create");
            targetFile.mkdirs();
        }
